...
async function main(): Promise<void> {
  ...
  const refFiliereRoot = await prisma.referential.upsert({
    ...
    update: {},
    create: { tenantId: 'CI', refType: ReferentialType.FILIERE, refCode: 'SECONDAIRE_CI', labelFr: "Filieres du secondaire - Cote d'Ivoire", metadata: { source: 'MENET-DOB', annee: 2026 }, sortOrder: 0, status: EntityStatus.ACTIVE },
  });
  ...
  for (const [index, filiere] of filieres.entries()) {
    await prisma.referential.upsert({
      ...
      update: {},
      create: { tenantId: 'CI', refType: ReferentialType.FILIERE, refCode: filiere.refCode, labelFr: filiere.labelFr, metadata: filiere.metadata, sortOrder: index + 1, parentId: refFiliereRoot.id, status: EntityStatus.ACTIVE },
    });
  }
  console.log('OK Referentiels filieres BEPC CI crees');

  // ETAPE 8 - SERVICE ZOUGLOU (service 1 deja cree)
  ...
  // ETAPE 9 - MENUS USSD (Zéro Hardcode)
  await setTenantContext('CI', superAdmin.id);
  ...
  for (const menu of menusUssd) {
    await prisma.referential.upsert({
      ...
      create: { tenantId: 'CI', refType: ReferentialType.METIER, refCode: menu.refCode, labelFr: menu.labelFr, metadata: menu.metadata, sortOrder: (menu.metadata as any).ordre ?? 0, status: EntityStatus.ACTIVE },
    });
  }
  console.log('OK Menus USSD crees : ' + menusUssd.length + ' referentiels');

  // ETAPE 10 - 36 SERVICES VAS MANQUANTS
  const services = [
    // PORTE 1
  ...
  for (const svc of services) {
    await prisma.yiraConfigService.upsert({
      ...
      update: { ussdPath: svc.ussdPath },
      create: {
        tenantId: 'CI',
        serviceCode: svc.serviceCode,
        serviceName: svc.serviceName,
        ussdPath: svc.ussdPath,
        vasGroup: svc.vasGroup,
  ...
  console.log('OK ' + createdCount + ' services VAS crees (36 nouveaux + ZOUGLOU = 37 total)');

  // RESUME FINAL
  console.log('\nSeed base_core termine avec succes.');
  ...
  console.log('Referentiels: SEC_A, SEC_C, SEC_D + ' + menusUssd.length + ' menus USSD');
